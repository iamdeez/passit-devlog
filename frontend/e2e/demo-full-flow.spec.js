...
/**
 * 전체 플로우 데모 시나리오 - 백엔드 없이 Frontend만으로 실행
 *
 * 테스트 시나리오:
 * 1. 판매자: 티켓 등록 → 목록 조회 → 상세 조회
 * 2. 판매자: 문의 생성 → 목록 조회 → 상세 조회
 * 3. 구매자: 티켓 상세 → 채팅 시작 → 메시지 전송
 * 4. 구매자: 티켓 상세 → 양도 요청
 */

test.describe("전체 플로우 데모 (Frontend Only)", () => {
  // 긴 타임아웃 설정 (데모용)
  test.setTimeout(300000); // 5분

  test("전체 플로우 - 티켓 등록부터 양도 요청까지", async ({ page }) => {
    ...
    // ========================================
    console.log("\n" + "─".repeat(80));
    console.log("📍 PART 1: 판매자 - 티켓 등록 플로우");
    console.log("─".repeat(80));
    ...
    const firstTicket = page
      .locator('div[role="button"], a[href*="/tickets/"], .MuiCard-root')
      .first();
    ...
    if (await createInquiryButton.isVisible({ timeout: 5000 })) {
      ...
      if (await contentInput.isVisible({ timeout: 3000 })) {
        await contentInput.fill("티켓 환불은 어떻게 진행하나요? 상세한 절차를 알려주세요.");
        await page.waitForTimeout(500);
    ...
    // 첫 번째 문의 클릭 (상세 조회)
    const firstInquiry = page.locator('div[role="button"], a[href*="/inquiries/"]').first();
    ...
    // 티켓 상세 페이지로 다시 이동
    await page.goto("/tickets/1/detail");
    await page.waitForLoadState("networkidle");
    await page.waitForTimeout(2000);
    console.log("🎫 티켓 상세 페이지 로드 완료");
    ...
    if (await purchaseButton.isVisible({ timeout: 5000 })) {
      ...
      // 요청 확인 버튼 클릭
      const confirmButton = page.getByRole("button", { name: /요청|신청|확인/i }).last();
    ...
    console.log("📊 실행된 플로우:");
    console.log("   1. ✓ 판매자: 티켓 등록 → 목록 조회 → 상세 조회");
